...
* of investigating the nature of any linkages between the variables that may exist
...
Clearly if the results of any analysis are based on a **sample drawn from the population**, if the sample is going to have any validity, then the sample should be chosen in a way that is fair and reflects the structure of the population.
...
As soon as sample data is used, the information contained within the sample is *“Imperfect”* and depends on the particular sample chosen. The **key problem** is to use this sample data to draw valid conclusions about the population with knowledge of and taking into account the '*error due to sampling*'.
...
For an attribute variable it is very simple. We observe the frequency of occurrence of each level of the attribute variable as shown in the barplot above/previous page.
...
In general there are four different combinations of types of Response Variables and types of Explanatory Variables. These four combinations are shown below:

```{r RelationshipMatrix, out.width = "500px",  fig.align = 'center', echo=FALSE}
...
The method for investigating the connections between a response variable and an explanatory variable depends on the type of variables. The methodology is different for different combinations as shown in the box above, and applying an inappropriate method causes problems. 💡⚡️😩  
...
```{r DAMet, out.width = "500px",  fig.align = 'center', echo=FALSE}
...
Consider the model: $$Y = f(X) + e \text{		[e is the effect of all other variables]}$$
...
```{r MvMDAM, out.width = "500px",  fig.align = 'center', echo=FALSE}
...
'Further Data Analysis' is a procedure that enables a decision to be made, based on the sample evidence, as to one of two outcomes:  
...
<span style="color:darkred">The effect of all other variables is commonly abbreviated to **e**.</span>
...
In fact, the correlation is a parameter of the bivariate normal distribution, that is **used to describe the association** between two variables, which does not include a cause and effect statement. That is, one variable does not depend on the other, i.e. the variables are not labelled as dependent and independent. Rather, they are considered as two random variables that seem to vary together. Hence, it is important to recognise that _correlation does not imply causation_. In correlation analysis, both $Y$ and $X$ are assumed to be random variables, whilst in linear regression, $Y$ is assumed to be a random variable and $X$ is assumed to be a fixed variable.
...
To demonstrate the data analysis methodology we'll go to the Share Price Case Study.
...
```{r DAM_MvM2, out.width = "500px",  fig.align = 'center', echo=FALSE}
knitr::include_graphics("images/DAM_MvM.png")
```

As part of an informal investigation of the possible relationship between `Share_Price` and `Profit`, first we will use R to obtain a scatter plot with the line of the best fit. Rather than referring every time to the name of the data set containing the variables of interest, we will attach our data and refer to the variables directly using only their names (see help for the `attach( )` function). 
...
<span style="color:darkgray">Let's go through the code and the functions we have used to produce this graph.</span> 

<span style="color:darkgray">
- The `plot( )` function gives a scatterplot of two numerical variables. The first variable listed will be plotted on the horizontal axis and the second on the vertical axis, i.e. you 'feed' as the arguments first the variable representing X and then the variable representing Y: `plot( x, y)`.
Considering that we are investigating a relationship between $X$ and $Y$ in the form of a regression line $Y = b_0 + b1_X + e$, we specify that in **R** as a formula `Y ~ X`, which can be read as "$Y$ is modelled as a function of $X$". This means that in the **R**’s `plot( )` function a formula interface can also be used, in which case the response variable $Y$ needs to come before the tilde ($\sim$) and $X$ variable that will be plotted on the horizontal axis.</span>

<span style="color:darkgray">
- Next, we fit a **line of best-fit** through our scatterplot using the `abline( )` function for the linear model $Y = b_0 + b_1X$ to see how close the points are to the fitted line. The basic R function for **fitting linear models by ordinary least squares** is the `lm( )`, which stands for **l**inear **m**odel. All we need to feed R with when using the `lm( )` function is the formula `Y~X`.
</span>
...
$b_0$ & $b_1$ to which it refers as coefficients.</span>
...
We must now take this estimated model and ask a series of questions to decide whether or not our estimated model is good/bad: that is, we have to subject the fitted model to a set of tests designed to check the validity of the model, which is in effect a test of your viewpoint/theory as a modeller.
...
<span style="color:darkgray">
- **Call**: shows what function and variables were used to create the model. </span> 

<span style="color:darkgray">
- **Residuals**: the difference between what the model predicted and the actual value of Y. Try to see if you can calculate this 'residuals' section by yourself using: `summary(Y-model$fitted.values)`. 🤓
</span> 
...
+ $H_0 : R^2  = 0$ (There is no relationship between the response and the explanatory variable.)
...
```{r F-testF-dist, out.width = "500px",  fig.align = 'center', echo=FALSE}
knitr::include_graphics("images/F-dist.png")
```

The value of $F_{crit}$ is the value that divides the distribution with $95\%$ of the area to the left of the $F_{crit}$ coordinate, and $5\%$ to the right. The accepted terminology is $F_{crit}$ but this text will call this quantity $F_{calc}$ to signify it is a quantity obtained from statistical tables. 
...
If $F_{calc}  <  F_{crit}$ then it favours $H_0$, whilst if $F_{calc}  > F_{crit}$ then it favours $H_1$. This decision rule can be represented graphically as shown below:
...
, i.e. this model is statistically significant and we need to describe it.
...
For the data analysis situation Measured v Measured this is simply stating the line of best fit. This is a description of the connection between the two variables. Additionally the $R^2$ value should be quoted as this gives a measure of how well the data and the line of best fit match.
...
```{r Prediction_Rsq, out.width = "500px",  fig.align = 'center', echo=FALSE}
knitr::include_graphics("images/Predict_Rsq.png")
```
...
<span style="color:darkgrey">Avoid extrapolation! Refrain from predicting/interpreting the regression line for X-values outside the range of X in the data set!</span>
...
2) **Create a report in which you will:**
